...
#### Methods
- Since they are the same class, *ofxXRSSimpleButton*, Large Image Buttons have access to the same methods as [Large Buttons](docs/software/ofxXRS/components.md#large-button); however, methods that affect the drawing of the button, like `setShape()` and `setColor()`, will have no effect.
...
#### Methods
The folder has no special methods that have not already been detailed in [Common Methods](docs/software/ofxXRS/components.md#common-methods)
